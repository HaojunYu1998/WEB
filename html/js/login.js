...
function Register() {
    ...
    //用户名正则，4�?6位（字母，数字，下划线，减号�?    var uPattern = /^[a-zA-Z0-9_-]{4,16}$/;
    if(!uPattern.test(user_name)){
        alert("用户名必须是4-6位(字母，数字，下划线，减号)")
        return
    }
    //密码强度正则，最�?位，包括至少1个大写字母，1个小写字母，1个数字，1个特殊字�?    var pPattern = /^.*(?=.{6,})(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*? ]).*$/;
    if(!pPattern.test(passwd)){
        alert("密码最�?位，包括至少1个大写字母，1个小写字母，1个数字，1个特殊字")
        return
    }
    //Email正则
