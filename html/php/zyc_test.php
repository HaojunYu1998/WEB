...
error_reporting(E_ALL); ini_set('display_errors', 1);
	...
	function transfer($conn) {
		...
		if (mysqli_num_rows($res) == 0 or $src_user == $dst_user) {
			...
			//$src = mysqli_fetch_assoc(mysqli_query($conn, "SELECT amount FROM Guests WHERE user='$src_user'"));
			//$dst = mysqli_fetch_assoc(mysqli_query($conn, "SELECT amount FROM Guests WHERE user='$dst_user'"));
