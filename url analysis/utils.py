...
def collate_fn(batch):
	seq_tensor = [item[0] for item in batch]
	target_tensor = torch.LongTensor([item[1] for item in batch])
	seq_lengths = torch.LongTensor([item[2] for item in batch])
	raw_data = [item[3] for item in batch]

	return [seq_tensor, target_tensor, seq_lengths, raw_data]
...
def vectorized_data(data, item2id):
	...
	# transfer data(consisting of chars) into vectors(consisting of numbers, which are ids)
...
def create_dataset(data, input2id, target2id, batch_size=4):
	...
	return DataLoader(PaddedTensorDataset(seq_tensor, target_tensor), batch_size=batch_size, num_workers = 8)
